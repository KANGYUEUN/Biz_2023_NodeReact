...
/**
 * bbs API router 설정
 * 보통 API 서버는 view 가 없이 JSON(또는 XML) 데이터를 client 로 return 하는 서버를 말한다(Spring Rest Server)
 *
...
// multer를 사용하여 파일을 수신할때
// 파일이름을 latin1 방식으로 파일이름을 encoding 을 적용해 버린다
// windows 환경에서는 한글이름 파일이 깨지므로 변환시켜 준다.
const encKor = (str) => {
  console.log(str);
...
const storageOption = {
  filename: (req, file, cb) => {
    ...
    const filePrix = `${Date.now()}-${Math.round(Math.random() * 1e9)}`;
...
router.post("/insert", uploadMiddleWare.array("b_images"), async (req, res) => {
  ...
  // image 정보 생성
  // 대표이미지를 제외한 나머지 tbl_files 에 저장하기
...
router.get("/list", async (req, res) => {
  // include
  // sequelize 에서 1:N 관계가 설정 되어 있을때 자동 JOIN 하는 코드
...
//        req.params.seq  로 받기
// form 으로 전송한 데이터는
//        req.body 에 담겨서 통째로 받기
router.get("/detali/:seq", async (req, res) => {
