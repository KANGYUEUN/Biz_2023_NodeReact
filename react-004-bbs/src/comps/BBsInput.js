...
const BBsInput = () => {
  ...
  // type 이 file 인 input 에서 파일이 선택 되었을때 발생하는 event
  ...
  const filesChangeHandler = (e) => {
     ...
     * }
     *
     * 새로운 forEach(map, filter 등등이 포함) 는 사용 불가하다
     * images.forEach(image={})
     *
     * 만약, 유사배열을 새로운 forEach()로 사용하기 위해서는
     * 진짜 배열로 변환해 주어야 한다.
     * const 진짜배열 =  Array.from(유사배열) : 유사배열을 진짜 배열로 변환 해야 한다
     */
    ...
    Array.from(files).forEach((file) => {
      ...
      fileReader.onload = (fe) => {
        ...
        // 만약 files 개수가 10개 이면 setImages() 함수가 10번 실행되고
        ...
        // 그래서 setState() 에 데이터를 직접 주입 하지 않고
